...
export default function ExercisePicker({ onSelect, onClose, excludeIds = [] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black/40" onClick={onClose}>
      <div
        className="mt-auto bg-gray-900 rounded-t-2xl flex flex-col max-h-[85vh]"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-800">
          <h2 className="font-semibold text-gray-100">Elegir ejercicio</h2>
        ...
        <div className="px-5 pt-3 pb-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 text-sm rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 pb-6">
          ...
          {!loading && filtered.length === 0 && (
            <div className="text-center py-10 text-gray-500">
              <Dumbbell size={32} className="mx-auto mb-2 text-gray-500" />
              <p className="text-sm">No se encontraron ejercicios</p>
            </div>
          )}

          {!loading && filtered.length > 0 && (
            <ul className="pt-1 divide-y divide-gray-800">
              {filtered.map((ex) => (
                <li key={ex.id}>
                  <button
                    onClick={() => onSelect(ex)}
                    className="w-full flex items-center gap-3 py-3 text-left hover:bg-gray-800 transition-colors rounded-lg px-1"
                  >
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-100">{ex.name}</p>
                      {ex.muscle_groups && (
                        <p className="text-xs text-gray-500">{ex.muscle_groups.name}</p>
                      )}
                    </div>
                    {ex.equipment && (
                      <span className="text-xs text-gray-500 shrink-0">{ex.equipment}</span>
                    )}
                  </button>
                </li>
              ))}
